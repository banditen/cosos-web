...
Official marketing website for **COSOS** – AI chief of staff that keeps founders and lean teams focused on the moves that matter.
...
Values are centralized in [`src/lib/site-config.ts`](./src/lib/site-config.ts). Use exported helpers instead of reaching for `process.env` directly.
...
- Structured data placeholders for organization and product are injected via `<script type="application/ld+json">`.
...
- Replace `src` and `data-website-id` once a tracking provider (e.g., Umami, PostHog, Vercel Analytics) is chosen.
...
- CLI configuration lives in `components.json` and targets to `src` directory (`@/components`, `@/lib/utils`).
- Base utilities such as `cn` helper (`src/lib/utils.ts`) and primitives (`Button`, `Badge`, `Card`, `Separator`, `Sheet`) are available under `src/components/ui`.
- Tailwind is configured with shadcn token layers and `tailwindcss-animate` plugin while preserving the existing primary palette.
...
The command will respect repository aliases and Tailwind setup. Prefer reusing existing primitives when possible to keep the dependency footprint minimal.

After installing new components, run `npm run lint` and `npm run build` to ensure UI toolkit stays compatible with the deployment pipeline.
...
- **Updating theme tokens**:
  ...
  3. Validate dark/light legibility—`ThemeScript` toggles `dark` class on `<html>`.
...
2. Update [`AnalyticsPlaceholder`](./src/components/system/analytics-placeholder.tsx) with production script URL and site identifier.
...
- No extra configuration is required beyond environment variables and a Node 18+ runtime.
...
| Header + footer navigation links resolve to correct routes | ☐ |
